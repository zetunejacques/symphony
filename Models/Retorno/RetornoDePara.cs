namespace Symphony.Models
{
    public class RetornoDePara
    {
        public string CodExmLab { get; set; }
        public string DescExmLab { get; set; }
        public string DescMatLab { get; set; }
        public string CodExm { get; set; }
        public string DescExm { get; set; }
        public string CodMat { get; set; }
        public string DescMat { get; set; }
        public int Altura { get; set; }
        public int CodigoFormato { get; set; }
        public string Condicao { get; set; }
        public string Conservante { get; set; }
        public int Contrato { get; set; }
        public int Curva { get; set; }
        public int Glicemia { get; set; }
        public int Grafico { get; set; }
        public string Grupo { get; set; }
        public int Hematocrito { get; set; }
        public int Imagem { get; set; }
        public int LinfocitosAbsoluto { get; set; }
        public int MaterialDiversos { get; set; }
        public int Peso { get; set; }
        public int PesoAmostra { get; set; }
        public string Regiao { get; set; }
        public int TempoAmostra { get; set; }
        public int TempoDiurese { get; set; }
        public int VolumeDiurese { get; set; }

    }
}